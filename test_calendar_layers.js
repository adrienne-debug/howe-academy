...
console.log("\n── 📋 list pop-ups from the Today card; Store + Grabs on the Bank tab ──");
{
  ...
  ok("tapping a name always lands on My Day; the tab row is gone; idle still resets to Everyone/Routines", /if\(_kioskR\(\)\) kkTab="myday";/.test(slice("kkSel")) && /el\.style\.display="none"; el\.innerHTML=""; return;/.test(slice("renderKioskTabs")) && /kkTab="routines"; renderAll\(\); \}/.test(src));
