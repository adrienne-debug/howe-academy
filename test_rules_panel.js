...
const code = ["cbRenderRules", "capFor"].map(braceSlice).join("\n");
...
console.log("cbRenderRules — Rules + Max/day columns");
{
  ...
  if (h) {
    ...
    ok("header has Rules column and NO Max/day column (6c-2: Cap is the one number)", h.indexOf(">Rules<") >= 0 && h.indexOf(">Max/day<") < 0 && h.indexOf(">Cap<") >= 0);
    ok("token chip wired for sequential only", h.indexOf("rlSet('lincoln','ws','rulesTok','sequential'") >= 0 && h.indexOf("rulesTok','max1'") < 0);
    ...
    const onChips = (wsRow.match(/rulesTok','(sequential)',(?:true|false)\)" style="[^"]*background:#123456/g) || []).map(m => m.match(/rulesTok','([a-z0-9-]+)'/)[1]);
    ok("selected token highlighted (sequential on)", onChips.join(",") === "sequential", onChips);
    ok("no Max/day inputs anywhere (rulesUpdateMax retired)", h.indexOf("rulesUpdateMax(") < 0 && !/placeholder="wk"/.test(h));
    const rfRow = iR > iW ? h.slice(iR) : h.slice(iR, iW);
    ok("Cap stepper shown for the DAILY subject too (max1 → Cap 1 needs a door)", rfRow.indexOf("rlSet('lincoln','reflex','cap',") >= 0);
    ok("Reflex (rules max1, cap unset) shows effective cap 1", /rulesTok','sequential'[\s\S]*?/.test(rfRow) && /<b>1<\/b>/.test(rfRow.slice(0, rfRow.indexOf("rlSet('lincoln','reflex','minutes'"))));
    ok("Wordsmith (cap unset, plan-backed) shows effective cap 1", /<b>1<\/b>/.test(wsRow.slice(0, wsRow.indexOf("rlSet('lincoln','ws','minutes'"))));
  }
}
